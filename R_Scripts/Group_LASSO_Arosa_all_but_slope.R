...
lsl <- subset(lsl_std, select=-c(aspect, slope, flowdir, corine, ssgm, winter_P_30Y, veg_rough, lithology)) 
...
for (j in 1:k.boot ) {
  ...
  for (i in 1:n.folds) {
    ...
    assign(paste("model.1.no.slope.coeffiecients_cv_grpreg_grLasso_",i,"_",j,sep=""),  coefficients)
    
    # - write coefficients to output file
    sink(paste("/Volumes/Lauren_Uni/M3_SSGM/R_Modelling/",Site,"/",Site,".model.1.no.slope.coeffiecients_cv_grpreg_grLasso_",i,"_",j,".txt",sep=""))
    cat("============= coeffiecients ================\n")
    print(coefficients)
    cat("============= lambda.min ================\n")
    print(lambda.min)
    ...
    #                                        type="response")
    # 
    assign(paste("fcst.model.1.no.slope.cv.grpreg_grLasso.fold",i,"_",j,sep=""), fcst.model.1.cv.grpreg.fold)
    # 
    ...
    assign(paste("model.1.no.slope.cv.grpreg_grLasso.fold",i,"_",j,sep=""), fit.grpreg)
    
    # - save lambda plot to .pdf file
    pdf(paste("/Volumes/Lauren_Uni/M3_SSGM/R_Modelling/",Site,"/",Site,"_model_1_no.slope_cv_grpreg_grLasso_fold_",i,"_",j,".pdf",sep=""))
    plot(fit.grpreg)
    abline( h = l.se, col = "grey", lty = "dotted")
    abline( v = log( fit.grpreg$lambda[ idx.se ]), col = "grey30", lty = "dotted")
    dev.off()
  ...
  save(cv.grpreg, file=paste("/Volumes/Lauren_Uni/M3_SSGM/R_Modelling/",Site,"/",Site,".model.1.no.slope.cv.grpreg_grLasso_",j,".rda", sep=""))
  save(obs.grpreg, file=paste("/Volumes/Lauren_Uni/M3_SSGM/R_Modelling/",Site,"/",Site,".model.1.no.slope.lslpts.observations_",j,".rda", sep=""))
