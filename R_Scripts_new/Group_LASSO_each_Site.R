...
for (Site in names(Sites)) {
  ...
  print(str_interp("*** ${Site} ******"))

  out_dir_site <- file.path(out_dir, Site)
  dir.create(out_dir_site, showWarnings = FALSE)
  ...
  for (j in 1:k.boot ) {
    ...
    # TODO - Handle missing data better => Maybe throw away rows is better?

    #lsl.data.ID$lithology[is.na(lsl.data.ID$lithology)] <- "Sedimentgesteine"
    lsl.data.ID$gesteinsklasse[is.na(lsl.data.ID$gesteinsklasse)] <- mode(lsl.data.ID$gesteinsklasse)
    ...
    for (i in 1:n.folds) {
      ...
      sink(paste(out_dir_site,"/",Site,".model.1.Gestkl.coeffiecients_cv_grpreg_grLasso_",i,"_",j,".txt",sep=""))
      cat("============= coeffiecients ================\n")
      print(coefficients)
      cat("============= lambda.min ================\n")
      print(lambda.min)
  ...
  #pdf(paste("/Volumes/Lauren_Uni/M3_SSGM/R_Modelling/",Site,"/",Site,"_model_1_Gestkl_cv_grpreg_grLasso_fold_",i,"_",j,".pdf",sep=""))
    ...
    save(cv.grpreg, file=paste(out_dir_site,"/",Site,".model.1.Gestkl.cv.grpreg_grLasso_",j,".rda", sep=""))
    save(obs.grpreg, file=paste(out_dir_site,"/",Site,".model.1.Gestkl.lslpts.observations_",j,".rda", sep=""))
