...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Arosa/Arosa.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Arosa <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Arosa)
  load (paste(out_dir, '/Arosa/Arosa.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
# TODO Beni: Note wrong length here
print(paste("length(grpreg.model.Arosa): ", length(grpreg.model.Arosa)))
print(paste("length(obs.Arosa)", length(obs.Arosa)))

# -------- Baulmes
grpreg.model <- vector()
obs <- vector()

# Baulmes not all data available, yet.
# for ( j in 1:k.boot ) {
#   load (paste(out_dir, '/Baulmes/Baulmes.model.2.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
#   grpreg.model.Baulmes <- as.numeric(cv.grpreg)
#   grpreg.model <- c(grpreg.model, grpreg.model.Baulmes)
#   load (paste(out_dir, '/Baulmes/Baulmes.model.2.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Chrauchtal/Chrauchtal.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Chrauchtal <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Chrauchtal)
  load (paste(out_dir, '/Chrauchtal/Chrauchtal.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Hornbach/Hornbach.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Hornbach <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Hornbach)
  load (paste(out_dir, '/Hornbach/Hornbach.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Rappetal/Rappetal.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Rappetal <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Rappetal)
  load (paste(out_dir, '/Rappetal/Rappetal.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Turbach/Turbach.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Turbach <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Turbach)
  load (paste(out_dir, '/Turbach/Turbach.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Urseren/Urseren.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Urseren <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Urseren)
  load (paste(out_dir, '/Urseren/Urseren.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Val_Cluozza/Val_Cluozza.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Val_Cluozza <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Val_Cluozza)
  load (paste(out_dir, '/Val_Cluozza/Val_Cluozza.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
# 
# for ( j in 1:k.boot ) {
#   load (paste(out_dir, '/Val_Cluozza/Val_Cluozza.model.2.cv.grpreg_grLasso_',j,'.rda', sep=""))
#   grpreg.model.Val_Cluozza <- as.numeric(cv.grpreg)
#   grpreg.model <- c(grpreg.model, grpreg.model.Val_Cluozza)
#   load (paste(out_dir, '/Val_Cluozza/Val_Cluozza.model.2.lslpts.observations_',j,'.rda', sep=""))
#   obs.Val_Cluozza <- as.numeric(obs.grpreg)
#   obs <- c(obs, obs.Val_Cluozza)
# }
# 
...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Val_D_Entremont/Val_D_Entremont.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Val_D_Entremont <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Val_D_Entremont)
  load (paste(out_dir, '/Val_D_Entremont/Val_D_Entremont.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(out_dir, '/Val_Piora/Val_Piora.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Val_Piora <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Val_Piora)
  load (paste(out_dir, '/Val_Piora/Val_Piora.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
result_out_dir <- file.path(out_dir, "Model_Scores")
dir.create(result_out_dir, showWarnings = FALSE)
sink(paste(result_out_dir, "/Brier_Scores_grlasso_no_bootstrap_Gesteinsklasse.txt", sep=""))
...
sink(file.path(result_out_dir, "Brier_Scores_grlasso_bootstrap_Gesteinsklasse.txt"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Arosa_Gesteinsklasse.pdf"))
...
# pdf(file=file.path(result_out_dir, "/ROC_AUC_Baulmes_Gesteinsklasse.pdf"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Chrauchtal_Gesteinsklasse.pdf"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Hornbach_Gesteinsklasse.pdf"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Rappetal_Gesteinsklasse.pdf"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Turbach_Gesteinsklasse.pdf"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Urseren_Gesteinsklasse.pdf"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Val_Cluozza_Gesteinsklasse.pdf"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Val_D_Entremont_Gesteinsklasse.pdf"))
...
pdf(file=file.path(result_out_dir, "ROC_AUC_Val_Piora_Gesteinsklasse.pdf"))
...
# paste("Baulmes, AUC =",auc.baulmes),
labels=c(paste("Arosa, AUC =",auc.arosa), paste("Chrauchtal, AUC =",auc.chrauchtal), paste("Hornbach, AUC =",auc.hornbach),
...
pdf(file=file.path(result_out_dir, "ROC_AUC_All_in_One_Plot_Gesteinsklasse.pdf"))
par(cex.axis=1.3)
...
sink(file.path(result_out_dir, "Contingency_Table_results_Gesteinsklasse.txt"))
