...
factor_4_encode_aspect <- function(data) {
    ...
    data$aspect_factor[data$aspect > 315 & data$aspect <= 45] <- "N"
...
preprocess_after <- function(x) {
  return(x %>% handle_nan_values %>% one_hot_encode %>% mutate_at(numeric_features, scale))
}
...
example_data_rbf4 <- rbf_4_aspect(example_data)

p <- ggplot(data = example_data_rbf4) +
  geom_line(aes(x = aspect, y = aspect_northness), color='red') +
  geom_line(aes(x = aspect, y = aspect_eastness), color='blue') +
  geom_line(aes(x = aspect, y = aspect_southness), color='green') +
  geom_line(aes(x = aspect, y = aspect_westness), color='orange')
...
example_data_rbf8 <- rbf_8_aspect(example_data)

p <- ggplot(data = example_data_rbf8) +
  geom_line(aes(x = aspect, y = aspect_northness), color='red') +
  geom_line(aes(x = aspect, y = aspect_northeastness), color='blue') +
  geom_line(aes(x = aspect, y = aspect_eastness), color='green') +
  geom_line(aes(x = aspect, y = aspect_southeastness), color='orange') +
  geom_line(aes(x = aspect, y = aspect_southness), color='red') +
  geom_line(aes(x = aspect, y = aspect_southwestness), color='blue') +
  geom_line(aes(x = aspect, y = aspect_westness), color='green') +
  geom_line(aes(x = aspect, y = aspect_northwestness), color='orange')
...
res_ohe8 <- cv.block.training(lsl_data, 2, 2)
...
res_ohe4 <- cv.block.training(lsl_data, 2, 2)
...
res_rbf4 <- cv.block.training(lsl_data, 2, 2)
...
res_rbf8 <- cv.block.training(lsl_data, 2, 2)
...
names <- c(str_interp("${Site}$ohe8"), str_interp("${Site}$ohe4"), str_interp("${Site}$rbf4"), str_interp("${Site}$rbf8"))
...
boxplot(
  ...
  t="l", ylab="Brier Score", cex.axis=1.3, cex.lab=1.3, main="aspect experiments"
)

```
```{r}

plot_boxplot(res_ohe8.coef_frequentist, str_interp("Frequentist: ${Site}"))
