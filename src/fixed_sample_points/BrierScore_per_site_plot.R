...
data_dir <- "/Users/beni/Documents/BayianLasso/data/data/src/fixed_sample_points/out/lasso_fixed"
out_dir <- "/Users/beni/Documents/BayianLasso/data/data/src/fixed_sample_points/out/BrierScore_lasso_fixed"
...
for ( j in 1:k.boot ) {
  load (paste(data_dir, '/Arosa.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Arosa <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Arosa)
  load (paste(data_dir, '/Arosa.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(data_dir, '/Baulmes.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Baulmes <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Baulmes)
  load (paste(data_dir, '/Baulmes.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(data_dir, '/Chrauchtal.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Chrauchtal <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Chrauchtal)
  load (paste(data_dir, '/Chrauchtal.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(data_dir, '/Hornbach.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Hornbach <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Hornbach)
  load (paste(data_dir, '/Hornbach.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(data_dir, '/Rappetal.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Rappetal <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Rappetal)
  load (paste(data_dir, '/Rappetal.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(data_dir, '/Turbach.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Turbach <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Turbach)
  load (paste(data_dir, '/Turbach.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(data_dir, '/Urseren.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Urseren <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Urseren)
  load (paste(data_dir, '/Urseren.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
for ( j in 1:k.boot ) {
  load (paste(data_dir, '/Val_Cluozza.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Val_Cluozza <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Val_Cluozza)
  load (paste(data_dir, '/Val_Cluozza.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
#
# for ( j in 1:k.boot ) {
#   load (paste(data_dir, '/Val_Cluozza.model.2.cv.grpreg_grLasso_',j,'.rda', sep=""))
#   grpreg.model.Val_Cluozza <- as.numeric(cv.grpreg)
#   grpreg.model <- c(grpreg.model, grpreg.model.Val_Cluozza)
#   load (paste(data_dir, '/Val_Cluozza.model.2.lslpts.observations_',j,'.rda', sep=""))
#   obs.Val_Cluozza <- as.numeric(obs.grpreg)
#   obs <- c(obs, obs.Val_Cluozza)
# }
#
...
for ( j in 1:k.boot ) {
  # load (paste(data_dir, '/Val_D_Entremont.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  load (paste(data_dir, '/Val_Piora.model.1.Gestkl.cv.grpreg_grLasso_',j,'.rda', sep=""))
  grpreg.model.Val_D_Entremont <- as.numeric(cv.grpreg)
  grpreg.model <- c(grpreg.model, grpreg.model.Val_D_Entremont)
  # load (paste(data_dir, '/Val_D_Entremont.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
  load (paste(data_dir, '/Val_Piora.model.1.Gestkl.lslpts.observations_',j,'.rda', sep=""))
...
sink(paste0(out_dir, "/Brier_Scores_grlasso_no_bootstrap_Gesteinsklasse.txt"))
...
pdf(file=paste0(out_dir, "/bootstrapped_BS_all_Sites_noCorine.pdf"))
